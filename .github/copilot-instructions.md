...
- Filter behavior is intentionally permissive by default: with no filters configured, every SMS is forwarded. Include/exclude evaluation is centralized in `SmsFilterProcessor`; change filtering rules there rather than duplicating logic in receivers or UI. Within a single filter, a blank `sender` or `keyword` field acts as a wildcard ("any value"); both fields must match (AND logic) for the filter to apply. EXCLUDE filters always have priority over INCLUDE filters.
- SMS log retention is enforced inside `SmsLogDao.insert()`, not in UI code. `insert()` is a `@Transaction` default interface method that calls `insertRaw()` followed by `enforceMaxSmsLimit()` — do not bypass it with `insertRaw()` directly. The `maxSmsToKeep` setting from `EmailConfig` controls pruning after each insert.
- `SmsReceiver.onReceive()` calls `goAsync()` and launches a coroutine on `Dispatchers.IO` before returning. The `PendingResult` from `goAsync()` must be finished in a `finally` block to prevent Android from killing the process before the email is sent and the log entry is saved. Preserve this pattern when modifying `SmsReceiver`.
- `EmailSender.sendEmail()` returns Italian result strings. Success is detected in `SmsReceiver` via `result.startsWith("Email inviata con successo")`. Do not change that prefix without also updating the check in `SmsReceiver`. To enable verbose JavaMail logging, flip the `DEBUG` constant in `EmailSender.Companion`.
